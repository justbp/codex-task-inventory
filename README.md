...
启动待办时，看板通过 Codex 原生 deep link 打开指定工作目录的新对话，并预填用户填写的标题和说明。用户在 Codex App 发送后，看板识别该真实 `threadId`，把项目、标签、优先级、排序、置顶和备注迁移到该线程，并将原手工记录标记为已绑定。看板之后只展示真实 Codex 线程，卡片会随运行态进入“进行中”和“待 Review”。
